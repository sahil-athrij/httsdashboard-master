...
def AvgCasePerWeekday(df_inqueue_events,df_engr_by_date,start_date,end_date,filename):
    ...
    ax = df_avgcnt_weekdy.plot(kind='bar',x='WeekdayName',y=['AvgCaseVol','AvgEngrNo'],figsize=(5,5),title='AvgCasePerWeekday '+start_date+'-'+end_date)
    ax.set_xlabel("")
    ax.set_ylabel("")
...
def TotCase(df,start_date,end_date,filenames=[]):
    ...
    df_totcase_wg_2.plot(kind='bar',stacked=True,figsize=(15,5),title='Total InQueue Case '+start_date+'-'+end_date).get_figure().savefig(filenames[0],bbox_inches='tight',pad_inches = 0)
    ...
    df_totcase_wg_3[['CX-APJC-BLR-ACI-SSPT','CX-APJC-SYD-ACI-SSPT']].plot.area(stacked=False,figsize=(15,5),title='InQueue Case Volume BLR/SYD '+start_date+'-'+end_date).get_figure().savefig(filenames[1],bbox_inches='tight',pad_inches = 0)
    ...
    df_totcase_wg_3_2[df_totcase_wg_3_2['Ratio'] < 100].plot.area(x='Date',y='Ratio',figsize=(15,5),title='BLR/SYD InQueue LB Actual Ratio '+start_date+'-'+end_date,grid=True).get_figure().savefig(filenames[2],bbox_inches='tight',pad_inches = 0)
...
def CasePerHour(df,start_date,end_date,filenames=[]):
    ...
    while cur_datetime <= end_datetime:
        ...
        df_case_by_hour = DFCasePerHourByDate(df_case_by_hour,df,start_hour=shift_hour[0],date=cur_datetime.strftime("%Y-%m-%d"))
    ...
    df_case_by_hour.set_index('Date').plot.area(y=['H1','H2','H3','H4','H5','H6'],figsize=(15,5),stacked=True,title='Case Volume per hour '+start_date+'-'+end_date).get_figure().savefig(filenames[0],bbox_inches='tight',pad_inches = 0)
    df_case_by_hour.set_index('Date').plot(y=['H123','H456'],figsize=(15,5),kind='bar',stacked=True,title='Case Volume per 3 hours '+start_date+'-'+end_date).get_figure().savefig(filenames[1],bbox_inches='tight',pad_inches = 0)
...
def AvgTakenCasePerEngr(df,start_date,end_date,filenames=[]):
    ...
    df_engr_by_date.plot.area(x='Date',y=['BLREngrNo','SYDEngrNo'],figsize=[15,5],stacked=False,title='Onshift Engineer '+start_date+'-'+end_date).get_figure().savefig(filenames[1],bbox_inches='tight',pad_inches = 0)
    ...
    df_caseperengr_compare.plot.area(x='Date',y=['BLRCasePerEngr','SYDCasePerEngr'],figsize=[15,5],title='Case Taken Per Engineer '+start_date+'-'+end_date,stacked=False).get_figure().savefig(filenames[0],bbox_inches='tight',pad_inches = 0)
    
    medianprops = dict(linestyle='-', linewidth=2.5, color='firebrick')
    ax = df_caseperengr_compare.plot(kind='box',x='Date',y=['SYDCasePerEngr','BLRCasePerEngr'],figsize=[5,5],title='CasePerEngrBox '+start_date+'-'+end_date,stacked=False,grid=True,medianprops = medianprops )
    ax.set_xlabel("")
    ax.set_ylabel("")
...
def main(start_date,end_date,force_overwrite=False):
    ...
    inqueue_filename = '/HTTSDashboard/logs/ACI/events/*_InQueueEvent.txt'
    accept_filename = '/HTTSDashboard/logs/ACI/events/*_AcceptEvent.txt'
    ...
    report_dir = '/HTTSDashboard/logs/ACI/analytics/'
    report_dir = report_dir+start_date+"_"+end_date+'/'
    if os.path.isdir(report_dir): 
        if force_overwrite:
            print("{} anlytics directory exists, force_to_overwrite.".format(report_dir))
        else:
            print("{} anlytics directory exists, return.".format(report_dir))
            return
    else:
        os.mkdir(report_dir)
        print("Creating analytics directory {}".format(report_dir))
    
    #Generate TotCase plot
    filename_totcase_all_queue = report_dir+'Total_Case_All_Queues.png'
    filename_totcase_syd_blr = report_dir+'Total_Case_SYD_BLR.png'
    filename_totcase_syd_blr_ratio = report_dir+'Total_Case_SYD_BLR_Ratio.png'
    #print("Generating Total Case Volume per day {} {}".format(filename_totcase_all_queue,filename_totcase_syd_blr))
    
    TotCase(df_inqueue_events,start_date,end_date,[filename_totcase_all_queue,filename_totcase_syd_blr,filename_totcase_syd_blr_ratio])
    
    #Case By Hour
    filename_caseperhour = report_dir+'Case_Per_Hour.png'
    filename_caseper3hours = report_dir+'Case_Per_3Hours.png'
    ...
    filename_caseperengr = report_dir+'Case_Taken_Per_Engr.png'
    filename_caseperengr_box = report_dir+'Case_Taken_Per_Engr_Box.png'
    filename_onshiftengr = report_dir+'OnShift_Engineer.png'
    df_engr_by_date = AvgTakenCasePerEngr(df_accept_events,start_date,end_date,[filename_caseperengr,filename_onshiftengr,filename_caseperengr_box])

    #Generate AvgCasePerWeekday plot
    filename_caseperweekday = report_dir+'Case_Per_Weekday.png'
...
if __name__ == "__main__":
    ...
    argparser.add_argument('--force_overwrite', action='store_true', help="Force to overwrite the analytics even it exists")
